...
export function FormAddImage({ closeModal }: FormAddImageProps): JSX.Element {
  ...
  const formValidations = {
    // validação de formatos aceitos e menores que 10 mb
    image: {
    ...
    // validação de menor e maior comprimento
    title: {
    ...
    // validação de máximo comprimento
    description: {
  ...
  const mutation = useMutation(
    ...
    {
      // se imagem subir -> recarregar imagens
      onSuccess: () => {
  ...
  const onSubmit = async (data: NewImageData): Promise<void> => {
    try {
      ...
      // mostrar toast de erro se submit falhar
      toast({
        status: 'error',
      ...
      // limpar formulários, estado e fechar modal
      reset();
      setImageUrl('');
      setLocalImageUrl('');
      closeModal();
